...
## 2026-07-24 이안이 전력질주 자세·상하 흔들림·다리 주기 안정화
...
발을 지면 기준선 위로 올렸다. 후속 피드백에서는 수직으로 선 상체 때문에 산책처럼 보이던
문제를 보정했다. 골반부터 머리까지 몸통 전체를 진행 방향으로 약 8~12도 숙이고 팔꿈치를
80~100도로 접어 앞뒤 스윙을 키워 전력질주 실루엣으로 바꿨다.

| 측정 항목 | 2026-07-23 교체본 | 2026-07-24 최종본 |
|---|---:|---:|
| 전체 실루엣 상단 편차 | 23px | 8px |
| 머리 수평 중심 편차 | 9.7px | 0.3px |
| 머리 수직 중심 편차 | 20.1px | 6.5px |
| 공중 프레임 발 높이 | 지면 고정 | 지면 위 3~8px |
...
- 프롬프트 핵심: 승인된 이안이의 얼굴·보울컷·체크 조끼·체형·화풍을 유지하고, 카메라와
  ...
  다리는 보존한 채 골반 축으로 상체를 8~12도 숙이고 반대쪽 팔·다리가 강하게 교차하도록 보정
- 생성 최종본:
  `output/imagegen/endless-runner-8frame-2026-07-24/checkered-vest-boy-soft-3d-toy-run-jump-8frame-sheet.png`
- 생성 최종본 SHA-256:
  `0db860bf36c2d89e0cca516602c1c09796824caa40c7c011d6c2ffb579e55145`
- 프로젝트 원화 SHA-256:
  `9431e7f39dece9d9dd50b9f41c26ff0373dd6fcf06d7e698f850f8bf8adc6b43`
- 런타임 결과:
  `endless-runner/assets/characters/checkered-vest-boy-soft-3d-toy/checkered-vest-boy-soft-3d-toy-run.gif`
- 런타임 GIF SHA-256:
  `f0969759fa31df3ef3779abf6ac43d90f603d1713a0765a410ff47e2aae46372`
...
## 2026-07-24 픽셀 히트박스와 캐릭터 높이 공정성 보정

실제 캐릭터 머리가 코인에 닿아도 획득되지 않던 원인은 코인 판정도 작은 공통 물리 박스
`30×42px`의 중심만 사용한 데 있었다. 장애물용 물리 박스는 두 캐릭터가 완전히 동일하게
유지하고, 코인 획득 박스만 각 동작 원화에서 투명 픽셀을 제외한 실루엣 경계로 분리했다.

- 실측 메타데이터:
  `endless-runner/assets/characters/runtime-metrics.json`
- 실루엣 기준: 256×256 원화의 `alpha >= 96`
- 달리기: GIF의 현재 프레임을 브라우저에서 알 수 없으므로 8프레임 합집합 경계 사용
- 점프·착지: 물리 상태와 동기화된 현재 PNG 프레임의 개별 경계 사용
- 슬라이드: `enter`, `hold`, `exit`가 실제 재생하는 프레임 묶음의 합집합 경계 사용
- 코인: 반지름 11px 원과 현재 실루엣 박스가 닿는 순간 획득
- 장애물: 달리기·점프 `30×42px`, 슬라이드 `30×22px` 공통 박스 유지

이엘이와 이안이의 달리기 프레임 평균 높이는 각각 187px, 179.625px이다. 이엘이는
`1.0`, 이안이는 `1.041058` 표시 배율을 사용해 보정 후 평균 높이를 동일하게 맞췄다.
배율은 발 피벗을 기준으로 적용돼 지면 접촉 위치에는 영향을 주지 않는다.

- 정적 자산·메타데이터 검증: PASS — 픽셀 경계, 합집합, 평균 높이와 배율 일치
- `npm run build`: PASS — Vite 7.3.6, 224 modules transformed
- 로컬 안엘런 Playwright: PASS — 14/14
- 추가 회귀: 머리 접촉 코인은 새 획득 박스에 포함되고 공통 장애물 박스에는 포함되지 않음,
  두 캐릭터의 보정 후 달리기 표시 높이와 장애물 박스가 동일함
