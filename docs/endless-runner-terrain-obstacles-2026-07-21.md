...
| 라운드 | 신규 요소 | 기본 회피 |
|---|---|---|
| 1 | 그루터기, 공중 잔디 지형, 줄을 타고 내려온 뱀 | 점프, 발판 착지, 슬라이드 |
| 2 | 이끼 바위, 가시밭, 다가오는 벌, 구덩이 | 점프, 슬라이드 |
| 3 이상 | 더 빠르게 다가오는 파랑새와 기존 장애물 혼합 증가 | 슬라이드 또는 높은 2단 점프 |

기존처럼 15초마다 라운드가 오르고 속도는 단계 및 라운드 내 경과 시간에 따라 증가한다.
장애물 최소 간격은 라운드마다 감소하되 `270px` 아래로 줄지 않는다. 코인은 생성 뒤 위치를
...
접근 속도로 플레이어 쪽을 향해 날아온다. 작은 상하 움직임, 날갯짓, 진행 반대쪽 잔상으로
...
| 안엘런 Playwright | PASS — 8/8, 공중 발판 착지와 벌의 지형 대비 추가 접근 속도 포함 |
