...
| 라운드 | 챕터 | 핵심 구성 |
|---|---|---|
| 1~10 | 초원 학교 | 단독 점프·슬라이드·이동 발판·점프 높이 비행몹·2단 점프를 하나씩 학습 |
| 11~20 | 숲속 리듬 | 연속 점프·2연속 발판·비행몹 무리·두 동작 전환 |
| 21~30 | 바위 협곡 | 넓은 구덩이·바위탑 릴레이·세 동작 조합 |
| 31~40 | 폭풍 고원 | 짧아진 반응시간에서 공중·발판·정밀 전환 혼합 |
| 41~50 | 별빛 정상 | 최고 속도에서 모든 연속 코스 종합 |

15초마다 라운드가 오르고 `50/50`에서 최고 난이도를 유지한다. 50개 라운드는 모두 고유한
챕터·이름·힌트·첫 보장 패턴·후속 패턴 풀을 가지며, 15·25·35·45라운드에는 직전보다 속도를
낮추고 간격을 넓힌 회복 구간이 있다. 장애물 최소 간격은 `300px` 아래로 줄지 않고, 라운드
내부 가속을 포함한 최종 속도는 약 `526px/s`에서 멈춘다. 각 라운드의 상세표와 외부 러너
게임 조사 근거는 `docs/endless-runner-50-round-design-2026-07-24.md`에 기록한다.
...
설정한다. 비행 생물은 지형 스크롤에 더해 플레이어 쪽으로 직접 날아오되 벌·파랑새 모두
최대 `128px/s`로 제한한다. 작은 상하 움직임, 날갯짓,
진행 반대쪽 잔상으로
...
에셋 프리로드, 지면 비율, 장애물 카탈로그, 50개 고유 프로필과 최고 라운드 상한, 네 회복
구간, 연속 코스 반응시간, 이동 발판 치수·이동량, 1단/2단 점프 도달 높이, 점프 높이 비행몹
판정, 코인 안전 동선, 충돌 종료, 2단 점프와 캐릭터 액션 동기화다.
...
| 안엘런 Playwright | PASS — 13/13, 50개 고유 라운드·최고 난이도 상한·모바일 안내·기존 플레이 회귀 포함 |
| 전체 Playwright | PASS — 125 passed, 조건부 방 초대 20 skipped, 실패 0 |
