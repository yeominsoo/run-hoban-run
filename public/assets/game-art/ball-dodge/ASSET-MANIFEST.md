...
`player-rabbit.webp`는 최종 용량 감사를 거쳐 1024×1024, 나머지 두 파일은 1254×1254 RGBA
WebP로 제공한다. 세 파일 모두 투명 모서리와 알파 채널을 확인했다. 브라우저가 이미지를 불러오지
못하면 기존 Canvas 원형 렌더러를 대체 경로로 사용한다.
