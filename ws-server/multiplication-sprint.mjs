...
const COUNTDOWN_MS = 3000;          // 로비 "게임 시작" 클릭 후 카운트다운(두더지 사냥과 동일)
const MULT_MIN = 2;
const MULT_MAX = 9;                 // 표준 구구단(2단~9단) 범위
const PROBLEM_INTERVAL_MS = 3000;   // 문제 하나당 노출 시간 — 안 풀어도 이 시간이 지나면 다음 문제로 넘어간다
const ROUND_MS = 45000;             // 라운드 길이(약 15문제)
 ...
 *   problemIndex: number,
 *   currentProblem: { a, b, shownAt } | null,
 *   roundEndsAt: number,
 ...
 * }
 */
export function registerMultiplicationSprintServer() {
  const rooms = new Map();
  /** ws -> { roomCode, token } */
  const wsIdentity = new Map();
  const ranking = createRankingStore('multiplication-sprint');
  ...
  function genRoomCode() {
    ...
    let attempt = 0, code;
    do {
      const digest = createHash('sha256').update(`multiplication-sprint-${base}-${attempt}`).digest('hex');
  ...
  function nextProblem(roomCode) {
    ...
    room.problemIndex++;
    const a = MULT_MIN + Math.floor(Math.random() * (MULT_MAX - MULT_MIN + 1));
    const b = MULT_MIN + Math.floor(Math.random() * (MULT_MAX - MULT_MIN + 1));
    room.currentProblem = { a, b, shownAt: Date.now() };
    for (const p of room.players) p.answeredThisProblem = false;
    broadcast(room, { type: 'problem', index: room.problemIndex, a, b });
  }
  ...
  wss.on('connection', (ws) => {
    ws.on('message', (raw) => {
        ...
        if (!room.started || room.phase === 'lobby') {
          ...
          send(ws, {
            ...
            game: {
              ...
              problem: room.currentProblem ? { index: room.problemIndex, a: room.currentProblem.a, b: room.currentProblem.b } : null,
      ...
      if (msg.type === 'answer') {
        ...
        const correctAnswer = room.currentProblem.a * room.currentProblem.b;
        if (Number.isInteger(value) && value === correctAnswer) {
  ...
  console.log('[multiplication-sprint-server] registered ws path: /multiplication-sprint');
