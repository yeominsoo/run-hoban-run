#!/usr/bin/env bash
# 할리갈리(/halligalli)를 WAS(58.228.188.17)에 반영하는 스크립트.
# 반드시 run-hoban-run 저장소 루트에서, WSL2(SSH 키가 있는 로컬 머신)에서 실행할 것.
...
echo "== 2. WAS로 ws-server 소스 동기화 (halligalli.mjs 포함) =="
...
echo "== 5. WAS의 nginx.conf(저장소 사본과 별개!)에 /halligalli location 추가 =="
$SSH 'grep -q "location /halligalli" ~/rps-tls/nginx.conf || \
  sed -i "/location \/mafia {/i\\
    location /halligalli {\\
        proxy_pass http://127.0.0.1:30081/halligalli;\\
...
echo "== 완료: wss://toris-arcade.duckdns.org:30080/halligalli 로 실제 접속해서 확인해볼 것 =="
