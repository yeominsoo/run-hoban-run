...
호스트가 시작하면 참가자마다 말 4개, 보드는 외곽 20칸 + 중앙/대각선 지름길짜리 공유 그래프로
표현된다(`yutnori-board.mjs`). 모든 플레이어는 같은 시작점(`outer-0`)에서 출발하고, 그 칸이 곧
도착점이다. 시작점에서는 지름길을 고르지 않고 정해진 외곽 방향으로만 출발하며, 나머지 코너에서만
외곽 진행/지름길 분기를 고른다. 할리갈리처럼
