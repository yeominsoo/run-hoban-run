...
const WS_URL = (() => {
  const c = import.meta.env.VITE_REVERSI_WS_URL as string | undefined;
  if (c) return c;
  return `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.hostname}:8787/reversi`;
})();

const NAME_KEY = 'run-hoban-run:reversi-nickname';
const SESSION_KEY = 'run-hoban-run:reversi-session';
...
<div class="rv-shell">
  ...
  <div class="rv-stage">
    <h1 class="rv-title">리버시</h1>
    <p class="rv-sub">1:1 실시간 대결! 상대 돌을 내 돌 사이에 끼워 뒤집고, 끝나면 더 많은 돌을 가진 사람이 승리.</p>

    <!-- Entry -->
    <div class="rv-panel" id="entry-panel">
      <div class="resume-banner hidden" id="resume-banner">
        <p class="status-text" id="resume-text">이전에 참여하던 대결이 있어요.</p>
        <div class="resume-actions">
          <button id="resume-btn" type="button" class="rv-btn primary">재입장하기</button>
          <button id="resume-dismiss-btn" type="button" class="rv-btn secondary">새로 시작</button>
        </div>
      </div>
      ...
      <div class="entry-section" id="create-section">
        <button id="create-btn" type="button" class="rv-btn primary">방 만들기</button>
      </div>
      ...
      <div class="entry-section hidden" id="join-section">
        ...
        <button id="join-btn" type="button" class="rv-btn primary">참가하기</button>
      </div>
      ...
      <div class="how-to-play">
        ...
        <ul class="how-to-play-list">
          ...
          <li>내 돌로 상대 돌을 일직선으로 감싸면 그 사이 돌이 전부 내 색으로 뒤집혀요.</li>
          <li>둘 곳이 없으면 자동으로 차례가 넘어가고, 양쪽 다 둘 곳이 없으면 게임 종료 — 더 많은 돌을 가진 사람이 승리!</li>
        </ul>
      </div>
    ...
    <div class="rv-panel hidden" id="waiting-panel">
      <div class="spinner" aria-hidden="true"></div>
      <p class="status-text" id="waiting-status">서버에 연결하는 중…</p>
      <button id="cancel-btn" type="button" class="rv-btn secondary">취소</button>
    </div>

    <!-- Waiting for opponent -->
    <div class="rv-panel hidden" id="waiting-opponent-panel">
      <div class="room-share" id="lobby-share">
        ...
        <button id="lobby-copy-btn" type="button" class="rv-btn secondary">공유하기</button>
      </div>
      <div class="spinner" aria-hidden="true"></div>
      <p class="status-text">상대가 들어오면 자동으로 시작해요…</p>
      <button id="waiting-opponent-cancel-btn" type="button" class="rv-btn secondary">나가기</button>
    </div>

    <!-- Countdown -->
    <div class="rv-panel hidden" id="countdown-panel">
      <p class="status-text">곧 대결이 시작됩니다!</p>
      <div class="ws-countdown-number" id="countdown-number">3</div>
    </div>

    <!-- 선공(흑) 결정전 -->
    <div class="rv-panel hidden" id="deciding-panel">
    ...
    <!-- Playing -->
    <div class="rv-panel wide hidden" id="playing-panel">
      <div class="rv-scores-row">
        <span class="rv-score-tag black" id="black-score-tag">● 호스트 2</span>
        <span class="rv-score-tag white" id="white-score-tag">○ 게스트 2</span>
      </div>
      <p class="rv-turn-msg" id="turn-msg"></p>
      <div class="rv-timer-track"><div class="rv-timer-fill" id="timer-fill" style="width:100%"></div></div>
    ...
    <!-- Game over -->
    <div class="rv-panel hidden" id="game-over-panel">
      <p class="set-over-result" id="game-over-banner"></p>
      <p class="rv-final-score" id="final-score"></p>
      <button id="game-over-leave-btn" type="button" class="rv-btn secondary">나가기</button>
    </div>

    <!-- Error -->
    <div class="rv-panel hidden" id="error-panel">
      <p class="status-text" id="error-text">게임 서버에 연결할 수 없습니다.</p>
      <button id="retry-btn" type="button" class="rv-btn primary">다시 시도</button>
    </div>
  </div>
</div>
`;
...
const blackScoreTag = document.getElementById('black-score-tag')!;
const whiteScoreTag = document.getElementById('white-score-tag')!;
const turnMsg = document.getElementById('turn-msg')!;
const timerFill = document.getElementById('timer-fill')!;
const rvBoard = document.getElementById('rv-board')!;
...
rvBoard.innerHTML = Array.from({ length: SIZE * SIZE }, (_, i) =>
  `<button type="button" class="rv-cell" data-row="${Math.floor(i / SIZE)}" data-col="${i % SIZE}"><span class="rv-disc"></span></button>`
).join('');
const cellEls = Array.from(rvBoard.querySelectorAll<HTMLButtonElement>('.rv-cell'));
cellEls.forEach((el) => {
...
// ── Reversi 보드 헬퍼(합법수 하이라이트는 클라이언트에서 미리 계산 — 최종 판정은 서버 권위) ──
function inBounds(r: number, c: number) { return r >= 0 && r < SIZE && c >= 0 && c < SIZE; }
function opponentOf(color: 'black' | 'white') { return color === 'black' ? 'white' : 'black'; }
function flipsFor(b: Disc[][], row: number, col: number, color: 'black' | 'white'): [number, number][] {
  ...
  for (const [dr, dc] of DIRS) {
    ...
    if (line.length > 0 && inBounds(r, c) && b[r][c] === color) all.push(...line);
  }
  return all;
}
function legalMoveSet(b: Disc[][], color: 'black' | 'white'): Set<string> {
  const set = new Set<string>();
  for (let r = 0; r < SIZE; r++) for (let c = 0; c < SIZE; c++) {
    if (flipsFor(b, r, c, color).length > 0) set.add(`${r},${c}`);
  }
  return set;
}
...
async function copyLink(code: string, btn: HTMLButtonElement) {
  const link = `${location.origin}/reversi/?room=${code}`;
  await shareRoomLink({ url: link, title: '리버시 초대', text: `리버시 대결 방(${code})에 초대할게요!`, btn });
}
...
function renderScores() {
  blackScoreTag.textContent = `● ${blackName} ${blackCount}`;
  whiteScoreTag.textContent = `○ ${whiteName} ${whiteCount}`;
}

function renderBoard() {
  const legal = phase === 'playing' && turn === myColor ? legalMoveSet(board, myColor) : new Set<string>();
  cellEls.forEach((el) => {
    const r = Number(el.dataset.row);
    const c = Number(el.dataset.col);
    const disc = el.querySelector('.rv-disc')!;
    const value = board[r]?.[c];
    disc.className = `rv-disc${value ? ` ${value}` : ''}`;
    el.classList.toggle('legal', legal.has(`${r},${c}`));
  });
}
...
function renderTurnMessage() {
  if (turn === myColor) {
    ...
    turnMsg.className = 'rv-turn-msg mine';
  } else {
    turnMsg.textContent = `${turn === 'black' ? blackName : whiteName}님 차례를 기다리는 중…`;
    turnMsg.className = 'rv-turn-msg';
  }
}
...
function handleServerMessage(msg: any) {
  switch (msg.type) {
    ...
    case 'match_start': {
      ...
      blackCount = msg.blackCount;
      whiteCount = msg.whiteCount;
    ...
    case 'move_made': {
      ...
      renderBoard();
      if (msg.isAuto) {
        chatWidget.addMessage('general', { name: '알림', text: '시간 초과 — 서버가 자동으로 수를 뒀어요.', mine: false, system: true });
      }
      break;
    }
    ...
    case 'pass': {
      chatWidget.addMessage('general', { name: '알림', text: `${msg.passedColor === 'black' ? blackName : whiteName}님은 둘 곳이 없어 차례를 넘겼어요.`, mine: false, system: true });
      startTurnTimerDisplay();
      break;
    }
...
function renderGameOver(msg: any) {
  ...
  gameOverBanner.textContent = !winnerToken
    ...
    : iWon
      ? '🏆 승리! 더 많은 돌을 차지했어요!'
      : '패배! 상대가 더 많은 돌을 차지했어요.';
  gameOverBanner.className = 'set-over-result ' + (!winnerToken ? '' : iWon ? 'win' : 'lose');
  finalScore.textContent = `● ${blackName} ${msg.blackCount} : ${msg.whiteCount} ${whiteName} ○`;
  resetGameState();
  setPhase('game_over');
}
